...
function PublicShell({
  ...
  return (
    <div className="landing" style={{ minHeight: '100vh' }}>
      <header
        className="topbar"
        style={{ borderBottom: `3px solid ${brand}`, background: 'white' }}
      >
      ...
      <div className="section" style={{ maxWidth: 800 }}>
        ...
        <p className="footer" style={{ border: 'none' }}>
          Create your own branded quotes & invoices free →{' '}
          <a href="/">hustledesk</a>
        </p>
      </div>
    </div>
...
function BillTo({
  ...
  client?: { name: string; company?: string; phone?: string; email?: string; address?: string };
  label?: string;
}) {
  return (
...
function PayBox({
  ...
  return (
    <div style={{ marginTop: '1.25rem' }}>
      ...
      {terms && <p className="help" style={{ marginTop: 8 }}>{terms}</p>}
    </div>
  );
}
